...
async fn system_budget_hook(
    ...
    // For a type that HAS a system budget, the type decides and the caller does
    // not get a say. For everything else the caller's choice is the only signal
    // there is, so a supplied budget stands and an absent one stays absent.
    //
    // This used to be "an explicit budget wins, the hook only fills a gap",
    // which sounds right and is wrong for exactly these three types. A budget
    // on an INCOME, TRADE or TRANSFER row is not a user preference — it is
    // determined by what the row IS, and ADR-017/ADR-030 rely on that to keep
    // transfers and trades out of budget rollups. Letting a caller pre-fill it
    // made a structural guarantee into something every call site had to
    // remember.
    //
    // And they did not. `transactionFormBody.ts` sends
    // `budgetId: values.budgetId || uncategorizedId` for every type, then
    // overrides it for INCOME alone — so TRANSFER and TRADE arrived pre-filled
    // with Uncategorized and this hook dutifully declined. Three live rows were
    // wrong that way (2026-08-16/18/20), while 80 other transfers created
    // through different paths were fine, which is what made it look like a
    // one-off rather than a rule that could not hold.
    if system_budget::has_system_budget(&row.tx_type) {
