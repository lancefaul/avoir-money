...
/// Whether this row's budget is decided by its type rather than by the caller.
///
/// Exposed so the ledger hook can tell "the caller chose nothing" from "the
/// caller's choice is irrelevant here" — two cases that look identical at the
/// call site and must not behave the same.
pub fn has_system_budget(tx_type: &str) -> bool {
    system_budget_name(tx_type).is_some()
}
