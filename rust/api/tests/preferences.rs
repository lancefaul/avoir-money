...
async fn a_preference_survives_reopening_the_database() {
    ...
    let got = call(&second, "GET", "/preferences", None).await.expect("read");
...
async fn writing_the_same_key_replaces_rather_than_appends() {
    ...
    for v in ["first", "second"] {
        call(&pool, "PUT", "/preferences", Some(json!({ "key": "k", "value": v })))
            .await
            .expect("write");
    }
    let got = call(&pool, "GET", "/preferences", None).await.expect("read");
...
async fn a_written_key_can_be_removed() {
    ...
    call(&pool, "PUT", "/preferences", Some(json!({ "key": "k", "value": "v" })))
        .await
        .expect("write");
    call(&pool, "DELETE", "/preferences/k", None).await.expect("delete");
    let got = call(&pool, "GET", "/preferences", None).await.expect("read");
    assert_eq!(got.body, json!({}));
}
