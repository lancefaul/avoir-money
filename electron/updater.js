...
function start({ dataDir, log = console, version } = {}) {
  ...
  autoUpdater.on('update-available', (info) => {
    ...
    autoUpdater.downloadUpdate().catch((err) => {
      const message = err?.message ?? String(err);
      log.info?.(`[updater] download failed (ignored): ${message}`);
  ...
  autoUpdater.on('update-downloaded', async (info) => {
    ...
    // Only now is the user told anything, because only now is there something
    // to decide. The update is on disk and verified; restarting is the whole
    // remaining cost.
    const { response } = await dialog.showMessageBox({
      ...
      cancelId: 1,
      title: 'Update ready',
      message: `Avoir Money ${info.version} is ready to install.`,
      detail: 'The update will be applied when you restart. Nothing is changed until then.',
    });
    if (response === 0) {
      // `autoInstallOnAppQuit` already covers the ordinary quit; this is the
      // explicit path. The backend is stopped by the shell's own quit handlers
      // either way, so the database is closed before anything is replaced.
