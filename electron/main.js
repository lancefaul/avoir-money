  ...
  return new Promise((resolve, reject) => {
    ...
    server = spawn(bin, [], {
      env: {
        ...
        AVOIR_TOKEN: TOKEN,
        // Tells the backend to watch the stdin pipe below and exit when it
        // closes. Set only here, because only this process holds that pipe —
        // see `watch_parent` in rust/server/src/main.rs for why the flag exists
        // rather than the backend sniffing whether stdin looks like a pipe.
        AVOIR_SUPERVISED: '1',
      },
      // stdin is a pipe we open, never write to, and never close. It is the
      // backend's orphan protection on every platform: the OS closes it when
      // this process dies by ANY route, including a crash or a kill, which is
      // precisely the case the unix PDEATHSIG and getppid checks cannot cover
      // on Windows.
      stdio: ['pipe', 'pipe', 'pipe'],
    });

    // The supervision pipe is write-only from here and never written to, so
    // the only event it can raise is an error when the child goes first. An
    // unhandled 'error' on a stream throws, which would turn an ordinary
    // backend exit into a crash of the shell.
...
function registerUpdateBridge() {
  ...
  ipcMain.handle('updates:download', async () => {
    // Same shape as the check: failures come back in the state rather than as
    // a rejection, because an Error crossing IPC arrives as a mangled string
    // and the state already carries the message.
    try {
      await updater.downloadNow();
    } catch {
      // recorded in state by `downloadNow`
    }
    return updater.status();
  });
