...
/**
 * Empire Dark — the Avoir palette, dark.
 *
 * Added 2026-08-09 alongside the Empire light rebuild. The source palette ships
 * a full dark ramp (`--avoir-dark-*`) and a `prefers-color-scheme` role block,
 * neither of which had anywhere to live: the DS is one theme per file, picked
 * explicitly, not a media query. This is that half of the palette.
 *
 * **The ramps are inverted, which is the existing dark-theme convention rather
 * than a choice made here.** In every dark theme `brand`/`accent`/status run
 * 50-darkest to 900-lightest, so the components asking for `accent600` get a
 * bright interactive green instead of a near-black one. `accent600` lands on
 * the palette's `primary-300` (72% L), which is where theme-dark puts its own
 * interactive green (73%), so the DS reads consistently across both.
 *
 * **`neutral100` is derived.** The palette's own ramp steps `dark-800` ->
 * `dark-700` by only 3.3%, and nothing sat at a usable distance above the card
 * surface, so a stop is interpolated at 27%. It was added to serve
 * `surfaceHover`, which has since moved to `brand100` with the rest of the
 * Empire family; the stop stays because the ramp still needs a value in that
 * gap. Same reason `neutral25` was invented for the old Empire: a role needed a
 * value the authored ramp had no stop for.
 *
 * **Status ramps anchor on `--avoir-*-on-dark`.** The palette's fifth role per
 * family exists precisely for this theme and has no light-mode slot; it lands
 * at 400, the position the vivid badge fills read. The four steps below it are
 * dark tints of the same hue and the three above are lighter text tints,
 * matching the shape theme-dark already uses (note the deliberate jump between
 * 300 and 400 — the ramp is two groups, not one gradient).
 *
 * The rail keeps `neutral900`, which here is the palette's own canvas
 * (`dark-850`, a green-black), so Empire Dark's chrome stays green without
 * needing the light theme's `brand800` trick.
 */
export const empireDarkTheme = createTheme(vars, {
  color: {
    ...
    secondaryButtonFrom: 'oklch(22.6% 0.019 171.9)', // = neutral0
    secondaryButtonTo: 'oklch(19.9% 0.015 172.2)', // = neutral50
    secondaryButtonHoverFrom: 'oklch(27% 0.021 172)', // = neutral100
    secondaryButtonHoverTo: 'oklch(22.6% 0.019 171.9)', // = neutral0
    secondaryButtonBorder: 'oklch(35.3% 0.027 165.8)', // = neutral200
    ...
    neutral0: 'oklch(22.6% 0.019 171.9)', // = avoir-dark-800 — cards
    neutral25: 'oklch(21.2% 0.017 172)', // derived: between neutral0 and neutral50
    neutral50: 'oklch(19.9% 0.015 172.2)', // = avoir-dark-850 — page canvas
    neutral100: 'oklch(27% 0.021 172)', // derived: hover needs a 4%+ step (see docblock)
    neutral200: 'oklch(35.3% 0.027 165.8)', // = avoir-dark-600
    neutral300: 'oklch(47.1% 0.032 166.6)', // = avoir-dark-500
    neutral400: 'oklch(58.8% 0.027 166.5)', // = avoir-dark-400
    neutral450: 'oklch(64.8% 0.029 166.3)', // derived
    neutral500: 'oklch(70.7% 0.031 166.1)', // = avoir-dark-300
    neutral600: 'oklch(76.9% 0.02 162.8)', // = avoir-dark-200
    neutral700: 'oklch(81.7% 0.018 164.8)', // derived
    neutral800: 'oklch(86.4% 0.016 166.7)', // = avoir-dark-100
    neutral900: 'oklch(19.9% 0.015 172.2)', // = neutral50 — the rail follows the canvas

    // -- Surfaces (= palette stops) --
    background: 'oklch(19.9% 0.015 172.2)', // = neutral50
    surface: 'oklch(22.6% 0.019 171.9)', // = neutral0 — cards lift off the canvas
    surfaceRaised: 'oklch(19.9% 0.015 172.2)', // = neutral50
    surfaceOverlay: 'oklch(19.9% 0.015 172.2)', // = neutral50
    ...
    controlHover: 'oklch(27% 0.021 172)', // = neutral100 — the ghost button stays neutral
    sidebarSurface: 'oklch(19.9% 0.015 172.2)', // = neutral900
    ...
    // -- Text (= palette stops) --
    textPrimary: 'oklch(86.4% 0.016 166.7)', // = neutral800 (11.30:1 on surface)
    textSecondary: 'oklch(76.9% 0.02 162.8)', // = neutral600
    textTertiary: 'oklch(64.8% 0.029 166.3)', // = neutral450
    textPlaceholder: 'oklch(47.1% 0.032 166.6)', // = neutral300
    textInverse: 'oklch(96.1% 0.009 161.4)', // = brand900 — lightest
    textOnBrand: 'oklch(19.9% 0.015 172.2)', // = neutral50 (dark on the bright green fill)
    onAccent: 'oklch(19.9% 0.015 172.2)', // = textOnBrand (unchanged)
    ...
    borderStrong: 'oklch(47.1% 0.032 166.6)', // = neutral300
    ...
    onSuccess: 'oklch(19.9% 0.015 172.2)', // = neutral50
    onWarning: 'oklch(19.9% 0.015 172.2)', // = neutral50
    onDanger: 'oklch(19.9% 0.015 172.2)', // = neutral50
    onInfo: 'oklch(19.9% 0.015 172.2)', // = neutral50
    onNeutral: 'oklch(19.9% 0.015 172.2)', // = neutral50

    // -- Overlay --
    overlay: 'rgba(0, 0, 0, 0.6)',

    // -- Input --
    inputBg: 'oklch(22.6% 0.019 171.9)', // = neutral0 (11.30:1 for its text)
    inputBgHover: 'oklch(27% 0.021 172)', // = neutral100 (+4.4 off the field)
    inputBgDisabled: 'oklch(19.9% 0.015 172.2)', // = neutral50 — recessed
    inputBorder: 'oklch(35.3% 0.027 165.8)', // = neutral200 — the field IS `surface`, so the border draws it
    inputBorderHover: 'oklch(47.1% 0.032 166.6)', // = neutral300
