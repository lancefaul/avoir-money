...
describe('findCombinations', () => {
  it('spots one line recorded as two entries', () => {
    // The Journey tickets: one $600.00 statement line, two $300.00 app rows.
    const combos = findCombinations(
      [row('bank', '2026-02-08', 838.8)],
      [row('app1', '2026-02-05', 419.4), row('app2', '2026-02-05', 419.4)],
    );
  ...
  it('requires an exact sum', () => {
    expect(
      findCombinations(
        [row('bank', '2026-06-10', 838.8)],
        [row('a', '2026-06-10', 419.4), row('b', '2026-06-10', 419.0)],
      ),
    ).toHaveLength(0);
  });
  ...
  it('never suggests a single row as a combination', () => {
    ...
    expect(
      findCombinations([row('bank', '2026-06-10', 419.4)], [row('a', '2026-06-10', 419.4)]),
    ).toHaveLength(0);
  });
  ...
  it('ignores parts dated outside the window', () => {
    expect(
      findCombinations(
        [row('bank', '2026-06-10', 838.8)],
        [row('a', '2026-06-10', 419.4), row('b', '2026-06-30', 419.4)],
      ),
    ).toHaveLength(0);
  });
