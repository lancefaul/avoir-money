...
/**
 * The startup window, and why writes are held during it.
 *
 * zustand wraps `setState` so EVERY state change persists, and its rehydrate is
 * asynchronous. Between the store being created and the server's answer
 * arriving, the store still holds DEFAULTS — so anything that writes in that
 * window writes the defaults straight over the user's real settings.
 *
 * Nothing exotic triggers it: `PageHeader` sets the page title from a mount
 * effect, and a `set()` is a write. Measured in the packaged 1.0.6 app, with
 * nobody touching anything — backend up at 12:38:49Z, stored
 * `hiddenAccountIds` became `[]` at 12:38:51.192Z.
 *
 * That is also why it looked like it survived "open and close" but not a
 * restart: reopening a window reuses a hydrated store, and only a fresh launch
 * runs the race.
 *
 * `loadStarted` matters as much as `loadSettled`. Suppressing on
 * "hasn't loaded yet" alone would silently discard writes from any caller that
 * never reads first, which is a worse bug than the one being fixed. There is
 * only a race to lose when a read is actually in flight.
 */
...
async function load(): Promise<Record<string, string>> {
  ...
  if (!inflight) {
    ...
    inflight = api.preferences
      ...
      .finally(() => {
        // Settled means the race is over, whether the answer arrived or the
        // request failed. A failed read must NOT hold writes forever — that
        // would turn one bad request into a session that silently saves
        // nothing.
        loadSettled = true;
        inflight = null;
      });
...
export const serverPreferenceStorage = {
  ...
  setItem: async (name: string, value: string): Promise<void> => {
    /*
     * Hold the write while the initial read is still out — and do not cache it
     * either.
     *
     * Anything writing here is writing the store's DEFAULTS, because the store
     * has not been told what the server holds yet. Sending it destroys the
     * user's settings, which is the whole bug: hidden accounts came back on
     * every restart while the value on the server stayed correct right up until
     * the moment the app started.
     *
     * Caching it is the same mistake one layer down, and the first version of
     * this fix made it. The cache is what hydration reads, so a default parked
     * there under the same key shadows the server's real value and the store
     * rehydrates from the very thing being suppressed — a silent write turns
     * into a silent read. Nothing is lost by dropping it: the cache exists only
     * to answer `getItem` during hydration, and the store's own in-memory state
  ...
  removeItem: async (name: string): Promise<void> => {
    ...
    // Same window, and a deletion is the most destructive thing that can happen
    // in it — a store that has not hydrated yet has no reason to be deleting
    // anything, so a removal here is spurious by construction.
    if (loadStarted && !loadSettled) return;
