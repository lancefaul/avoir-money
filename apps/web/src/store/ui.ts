...
import { serverPreferenceStorage } from './preferenceStorage.js';
...
interface UIState {
  ...
  /**
   * Redact every value that identifies a person, an institution or an amount.
   *
   * Persisted deliberately, on ADR-005's rule: this is a setting about how the
   * page is READ, not transient view state. Closing the laptop in a cafe and
   * reopening it is exactly the moment the mask must still be on, and a privacy
   * control that quietly lapses on restart is worse than none.
   */
  masked: boolean;
  toggleMasked: () => void;
...
export const useUIStore = create<UIState>()(
  persist(
    ...
    }),
    {
      ...
      /*
       * The SERVER, not localStorage. Chromium keys localStorage by origin and
       * the origin includes the port, which the backend re-picks on every
       * launch (ADR-036) — so every one of these settings silently reset each
       * time the app opened. See `preferenceStorage.ts`; 58 stranded stores
       * had accumulated before anyone noticed, and only because hidden
       * accounts was the one setting whose default differed from the user's.
       */
      storage: createJSONStorage(() => serverPreferenceStorage),
