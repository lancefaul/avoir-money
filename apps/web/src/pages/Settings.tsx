...
export default function SettingsPage() {
  ...
  /*
   * `?tab=` seeds the pane, so Settings can be linked AT rather than merely
   * navigated to — which is what lets the title bar's update icon point at
   * Software Updates. It seeds rather than controls: once open, switching tabs
   * is local state and does not rewrite the URL, because a settings pane is not
   * a place anyone wants their back button to walk through.
   *
   * An unrecognised name falls back rather than throwing. A stale link should
   * land on Settings, not on an error boundary.
   */
  const [tab, setTab] = useState<SettingsTab>(() => {
    /*
     * Read straight from the URL rather than through `useSearch`. This is a
     * one-time seed, not a subscription — the tab does not track the URL after
     * mount — and `useSearch` would make the whole page require router context
     * to render at all, which it did not before and which its tests do not
     * provide. `validateSearch` on the route still declares and sanitises the
     * parameter for anyone NAVIGATING to it.
     */
    const requested = new URLSearchParams(globalThis.location?.search ?? '').get('tab');
    return TAB_ITEMS.some((t) => t.value === requested) ? (requested as SettingsTab) : 'backups';
  });
