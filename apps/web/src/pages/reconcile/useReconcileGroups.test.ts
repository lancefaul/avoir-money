...
describe('useReconcileGroups', () => {
  ...
  describe('hints', () => {
    it('flags two identical unmatched statement rows as a possible duplicate', () => {
      const session = build(
        [row('r1', 'ACME CREAMERY', 3.75), row('r2', 'ACME CREAMERY', 3.75)],
        [],
      );
      const { result } = renderHook(() => useReconcileGroups(session, []));
    ...
    it('leaves a combination out of the per-row duplicate pass', () => {
      ...
      const session = build([row('r1', 'TM *ACME TOUR', -600, '2026-06-12')], []);
      const { result } = renderHook(() =>
        useReconcileGroups(session, [
          tx('a1', 'Ticketmaster', 300, '2026-06-10'),
          tx('a2', 'Ticketmaster', 300, '2026-06-10'),
        ]),
      );
  ...
  describe('combinations', () => {
    ...
    const oneBankTwoApp = () => build([row('r1', 'TM *ACME TOUR', -600, '2026-06-12')], []);
    const twoAppRows = [
      tx('a1', 'Ticketmaster', 300, '2026-06-10'),
      tx('a2', 'Ticketmaster', 300, '2026-06-10'),
    ];
    ...
    it('reports every row exactly once across all groups', () => {
      ...
      const session = build(
        [
          row('r1', 'TM *ACME TOUR', -600, '2026-06-12'),
