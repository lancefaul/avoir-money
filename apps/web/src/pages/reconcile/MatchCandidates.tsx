        ...
        {ranked.map(({ c, amountDelta, dayDelta }) => (
          <li key={c.id}>
            <label className={s.candidate}>
              ...
              <span className={s.candidateAmount}>
                <Sensitive label="amount">{formatCurrency(c.amount)}</Sensitive>
              </span>
            </label>
          </li>
        ))}
...
function RunningTotal({ selected, target }: { selected: number; target: number }) {
  ...
  return (
    <p className={inputStyles.fieldHelper}>
      Selected <Sensitive label="amount">{formatCurrency(selected)}</Sensitive> of{' '}
      <Sensitive label="amount">{formatCurrency(target)}</Sensitive> —{' '}
      {exact
        ? 'matches exactly'
