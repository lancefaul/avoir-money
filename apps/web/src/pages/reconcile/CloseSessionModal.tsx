        ...
        <div className={inputStyles.formStack}>
          {balanced ? (
            ...
            <button
              ...
              className={`${buttonStyles.btnBase} ${buttonStyles.btnMd} ${buttonStyles.btnPrimary}`}
            >
              Set starting balance to {formatCurrency(correctedOpening)}
            </button>
          ) : (
            <button
              type="button"
      ...
      <div className={s.closeBody}>
        <div className={s.closeSummary}>
          <div className={s.closeRow}>
            <span>Bank says</span>
            <span>{formatCurrency(residual.statementEndingBalance)}</span>
          </div>
          <div className={s.closeRow}>
            <span>App says</span>
            <span>{formatCurrency(residual.expectedBalance)}</span>
          </div>
          <div className={s.closeRow}>
            <strong>Difference</strong>
            <strong>{formatCurrency(residual.residual)}</strong>
          </div>
        </div>
          ...
          <>
            <div className={s.closeWarning}>
              <span>
                ...
                {formatCurrency(Math.abs(residual.residual))} is unexplained.
              </span>
              <span>
            ...
            {showOpeningFix ? (
              <div className={s.openingPreview}>
                <span className={s.periodRange}>
                  Starting balance {formatCurrency(residual.openingBalance)}
                  <ArrowRight size={14} aria-label="becomes" />
                  <strong>{formatCurrency(correctedOpening)}</strong>
                </span>
                <span>
