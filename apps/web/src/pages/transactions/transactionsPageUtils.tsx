    ...
    <>
      <div className={ss.stat}>
        ...
        <span className={`${ss.statValue} ${ss.statNegative}`}>
          <Sensitive label="amount">{formatCurrency(summary.totalSpent)}</Sensitive>
        </span>
      </div>
      <div className={ss.stat}>
        <span className={ss.statLabel}>Spent/mo</span>
        <span className={`${ss.statValue} ${ss.statNegative}`}>
          <Sensitive label="amount">{formatCurrency(summary.avgSpent)}</Sensitive>
        </span>
      </div>
      <div className={ss.stat}>
        <span className={ss.statLabel}>Earned</span>
        <span className={`${ss.statValue} ${ss.statPositive}`}>
          <Sensitive label="amount">{formatCurrency(summary.totalEarned)}</Sensitive>
        </span>
      </div>
      <div className={ss.stat}>
        <span className={ss.statLabel}>Earned/mo</span>
        <span className={`${ss.statValue} ${ss.statPositive}`}>
          <Sensitive label="amount">{formatCurrency(summary.avgEarned)}</Sensitive>
        </span>
      </div>
    </>
  );
