...
export function useTransactionForm(options: UseTransactionFormOptions): UseTransactionFormReturn {
  ...
  /*
   * What the transaction being edited already takes from each account, in cents.
   *
   * An account's balance ALREADY has this transaction applied, so from the
   * editor's point of view that money is not spent — it is this row's own, and
   * re-spending it is what the edit is for. Without adding it back, a gift card
   * drained to zero by this very purchase reports a $0 cap and its leg clamps
   * to nothing, with no way to type the real figure back in.
   *
   * Empty when creating: a new transaction has taken nothing yet.
   */
  const [ownLegCents, setOwnLegCents] = useState<Record<string, number>>({});
  ...
  function availableCents(accountId: string): number | null {
    ...
    // `+ ownLegCents` is what makes editing possible at all. The stored balance
    // already reflects this transaction, so spending its own portion again is
    // not an overdraw — see `ownLegCents`.
    const spendable = Math.round((a.balance ?? 0) * 100) + (ownLegCents[accountId] ?? 0);
  ...
  function openCreate(defaultType?: FormValues['type']) {
    ...
    // A new row has taken nothing yet, so it gets no allowance. Leaving a stale
    // one here is the dangerous direction: it would let a create spend money the
  ...
  function openEdit(tx: CoreTransaction) {
    ...
    setLegAmounts({});
    setRewardsAmounts({});
    // Same reasoning as a re-split, one leg instead of several: an ordinary
    // edit on an account this transaction emptied must still be able to keep
    // its own amount.
    setOwnLegCents(tx.accountId ? { [tx.accountId]: Math.round(Number(tx.amount) * 100) } : {});
  ...
  function openDuplicate(tx: CoreTransaction) {
    ...
    // A duplicate is a NEW row. The original it was copied from still exists and
    // still holds its money, so none of it is available twice.
    setOwnLegCents({});
    setTradeError('');
  ...
  function openResplit(anchor: CoreTransaction, legs: ResplitLeg[]) {
    ...
    setOwnLegCents(Object.fromEntries(legs.map((l) => [l.accountId, l.amountCents])));
    setRewardsAmounts({});
    reset({
      type: 'EXPENSE',
      date: today,
