import { useMasked, REDACTED } from '@budget-tracker/ui';
...
export default function HoldingsPanel({
  ...
  const allocationHelper = allocationGroups
    .map((g) =>
      masked ? `${g.label} ${REDACTED}` : `${g.label} ${Math.round((g.value / totalValue) * 100)}%`,
    )
    .join(' · ');
        ...
        {hasHoldings && (
          <>
            <SegmentedProgress
              ...
              valueLabel={masked ? REDACTED : formatCurrency(totalValue)}
              size="lg"
