        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <ProgressBar
            ...
            valueLabel={
              masked
                ? REDACTED
                : `${money(d.originalBalance - d.currentBalance)} / ${money(d.originalBalance)}`
            }
            helper={masked ? `${REDACTED} remaining` : `${money(remaining)} remaining`}
          />
        </div>
          ...
          <tbody>
            <tr className={pp.row}>
              ...
              <td className={cn(pp.cell, pp.amountCell)} style={{ paddingRight: TEXT_LINE_RIGHT }}>
                <Sensitive label="amount">{formatCurrency(d.monthlyPayment)}</Sensitive>
              </td>
            </tr>
            <tr className={pp.row}>
              <td className={cn(pp.cell, pp.secondaryCell)} style={{ paddingLeft: TEXT_LINE_LEFT }}>
              ...
              <td className={cn(pp.cell, pp.amountCell)} style={{ paddingRight: TEXT_LINE_RIGHT }}>
                <Sensitive label="interest rate">{d.apr}%</Sensitive>
              </td>
            </tr>
            <tr className={pp.row}>
              <td className={cn(pp.cell, pp.secondaryCell)} style={{ paddingLeft: TEXT_LINE_LEFT }}>
