...
import { RefreshCw, Loader2, Package, Globe, Copy, Check, Clock } from 'lucide-react';
import {
  buttonStyles,
  badgeStyles,
  spinnerStyles,
  ...
  switch (state.status) {
    case 'available':
      return {
        ...
        description: 'It downloads in the background. Nothing installs until you restart.',
      };
    case 'downloading':
...
export default function SoftwareUpdates() {
  const { supported, state, history, checking, check, install } = useUpdates();
  /*
