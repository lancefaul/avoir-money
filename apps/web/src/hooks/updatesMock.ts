...
export function mockUpdatesBridge() {
  ...
  return {
    ...
    // The mock's state is fixed for the page load, so a download resolves to
    // the same thing a check does — enough to exercise the button's wiring
    // without pretending to fetch anything.
