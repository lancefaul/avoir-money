...
/*
 * The hook remembers the announced version through the server-backed
 * preference store, not localStorage — Chromium keys localStorage by origin and
 * the desktop's origin changes every launch, so "remember" wrote somewhere the
 * next launch could not read and the toast fired again every time.
 *
 * Stubbed with a plain map here so the assertions are about the version-keyed
 * logic rather than about a network call.
 */
const prefs = new Map<string, string>();
vi.mock('../store/preferenceStorage.js', () => ({
  serverPreferenceStorage: {
