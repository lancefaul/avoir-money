...
describe('TitleBar — with a shell', () => {
  ...
  it('keeps close flush to the corner, with the added controls on the left', () => {
    ...
    // every control added since has gone on the LEFT rather than beside it.
    //
    // The privacy toggle sits between the update mark and refresh. It is always
    // rendered, unlike the update mark, so it cannot shift its neighbours — the
    // rule this test exists to protect is satisfied by construction there.
    shell();
    render(<TitleBar />);
    const labels = Array.from(document.querySelectorAll('button')).map((b) =>
      b.getAttribute('aria-label'),
    );
    expect(labels).toEqual(['Hide values', 'Refresh', 'Minimise', 'Maximise', 'Close window']);
  });
