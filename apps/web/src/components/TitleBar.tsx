  ...
  return (
    <div className={s.bar}>
      ...
      <div className={s.controls}>
        {/*
          The update mark goes first, before Refresh, for the same reason
          Refresh goes before the window controls: minimise/maximise/close keep
          the corner positions muscle memory expects, and close in particular
          stays flush to the corner. It renders nothing at all when there is no
          update, so the three controls do not shift under the pointer as its
          state changes — a control that appears is acceptable in chrome; one
          that MOVES the close button is not.
        */}
        <UpdateButton />
        {/*
          The privacy control, between the update mark and Refresh. Always
          rendered, so unlike the update mark it never shifts its neighbours —
          the close button keeps its corner without anyone having to remember.
        */}
        <MaskButton />
        {/*
