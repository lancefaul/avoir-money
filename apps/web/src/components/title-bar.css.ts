...
export const button = style({
  ...
  width: '2.875rem',
  // Explicit, because one of these buttons is not a direct child of `controls`:
  // the update button is wrapped by the DS Tooltip's own `inline-flex` span, so
  // the parent's `align-items: stretch` reaches the span and stops there. The
  // siblings are already full height by stretch, so this changes nothing for
  // them and keeps the wrapped one from sitting short.
  height: '100%',
  border: 'none',
  ...
  // The positioning context for `updateProgress`, which the update button draws
  // along this button's bottom edge.
  position: 'relative',
  selectors: {
    '&:hover': {
...
/**
 * The update button when the download is on disk and waiting.
 *
 * Brand, not `success` or `warning`. DESIGN.md reserves the semantic families
 * for meaning — nothing has succeeded and nothing is wrong — and brand is what
 * the system already uses for "this is the thing to act on". It is also the one
 * mark in the chrome that should draw a glance, which is exactly what an accent
 * is for.
 */
export const updateReady = style({
...
/**
 * Download progress, as a hairline along the bottom edge of the button.
 *
 * Absolutely positioned against the button, which is why `button` above is a
 * flex container with no `position` of its own — this adds it. Two pixels, so
 * it reads as a fill rather than a rule, and flush to the bottom so it sits on
 * the boundary between the title bar and the page instead of floating inside
 * the control.
 *
 * `width` is set inline from the percentage: it is a continuously changing
 * value, and a class per percent is not a thing worth generating.
 */
export const updateProgress = style({
  position: 'absolute',
  left: 0,
  bottom: 0,
  height: '0.125rem',
  background: vars.color.brand500,
  transition: `width ${vars.duration.normal} ${vars.easing.default}`,
});
