...
function OverUnderLabel({ data }: { data: SpendPredictionResponse }) {
  ...
  if (amount > 0) return <span className={s.overLabel}>{formatCurrency(amount)} over</span>;
  if (amount < 0)
    return <span className={s.underLabel}>{formatCurrency(Math.abs(amount))} under</span>;
  ...
  return (
    <>
      ...
      <div className={s.amountRow}>
        ...
        <span className={s.budgetAmount}>{formatCurrency(data.expectedPeriodSpend)}</span>
      </div>
      <p className={s.subtitle}>{formatCurrency(data.expectedPeriodSpend / data.totalDays)}/day</p>
