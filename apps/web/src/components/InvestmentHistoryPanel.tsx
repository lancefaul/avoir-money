  ...
  if (!compact)
    return (
      <span className={colorClass}>
        <Sensitive label="profit/loss">{exact}</Sensitive>
      </span>
    );
  return (
...
export default function InvestmentHistoryPanel() {
  ...
  const renderQuantity = (entry: HistoryEntry) => {
    if (entry.assetType !== 'BITCOIN')
      return <Sensitive label="share count">{entry.quantity.toLocaleString()}</Sensitive>;
    if (!compact) return <Sensitive label="quantity">{formatSats(entry.quantity)}</Sensitive>;
    return (
      <Tooltip content={formatSats(entry.quantity)}>
        <span>
          <Sensitive label="amount">{formatSatsCompact(entry.quantity)}</Sensitive>
        </span>
      </Tooltip>
    );
  };
                      ...
                      <tr key={entry.id} className={tl.row}>
                        ...
                        <td className={tl.cell} style={{ overflow: 'hidden' }}>
                          <span className={tl.nameCell}>
                            <Sensitive label="ticker">{symbolContent(entry)}</Sensitive>
                          </span>
                          {merge && (
                            <span className={iv.subline}>
                              <Sensitive label="custodian">{custodianContent(entry)}</Sensitive>
                            </span>
                          )}
                        </td>
