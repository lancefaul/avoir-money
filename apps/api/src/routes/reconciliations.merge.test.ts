...
describe('POST /reconciliations/:id/merge', () => {
  it('the Ticketmaster fixture: two rows collapse to one −$600.00 row split across both budgets, no $0 category', async () => {
    ...
    const a = await expenseOn(account.id, recreation.id, 300);
    const b = await expenseOn(account.id, gifts.id, 300);
    ...
    const row = await addStatementRow(sessionId, -600);
    ...
    expect(Number(parent.amount)).toBe(600);
    ...
    expect(Number(child.amount)).toBe(300);
