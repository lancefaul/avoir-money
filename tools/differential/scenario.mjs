...
export const scenario = [
  ...
  // ── Preferences ───────────────────────────────────────────────────────
  //
  // Added 2026-08-14 with the route itself. `/preferences` existed only in the
  // Rust side until then, and a route the reference does not implement is a
  // route this harness cannot compare — the omission blindness ERRORS.md
  // records, where 86/86 and 61/61 were both true and both silent about
  // `/sign-conventions` being missing from the port entirely.
  //
  // Every step below is driven, not merely reachable. A route that is CALLED
  // is not the same as a case that is CONSTRUCTED, and the write path is the
  // half where a well-formed response is most likely to be hiding a wrong
  // write.
  { name: 'preferences before anything is stored', method: 'GET', path: '/preferences' },
  {
    name: 'store a preference',
    method: 'PUT',
    path: '/preferences',
    body: { key: 'budget-tracker-ui', value: '{"state":{"sidebarCollapsed":true},"version":1}' },
    expectStatus: 200,
  },
  { name: 'the preference as served', method: 'GET', path: '/preferences' },
  {
    // The property that fails SILENTLY if it regresses: a second write must
    // overwrite, never append. Getting it wrong leaves two rows and the read
    // returns whichever the database hands back first — the escrow defect of
    // ADR-032, where the write succeeded and the read could not see it.
    name: 'write the same key again',
    method: 'PUT',
    path: '/preferences',
    body: { key: 'budget-tracker-ui', value: '{"state":{"sidebarCollapsed":false},"version":1}' },
    expectStatus: 200,
  },
  { name: 'the preference after overwriting', method: 'GET', path: '/preferences' },
  {
    // The refusal guard from ERRORS.md: this path is driven with GOOD input
    // above and returns 200, so a 400 here cannot be two backends agreeing
    // about a route neither has.
    name: 'a preference with an empty key',
    method: 'PUT',
    path: '/preferences',
    body: { key: '', value: 'x' },
  },
  { name: 'forget a key that was never stored', method: 'DELETE', path: '/preferences/absent' },
  { name: 'forget the stored key', method: 'DELETE', path: '/preferences/budget-tracker-ui' },
  { name: 'preferences after forgetting', method: 'GET', path: '/preferences' },
